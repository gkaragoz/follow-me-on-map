...
class MockDataService {
  ...
  MockDataService(this._storageService);

  /// Seeds mock sessions if the database is empty.
  Future<void> seedIfEmpty() async {
    final existing = _storageService.getAllSessions();
  ...
  /// ~2.5 km morning jog along the Bosphorus coast (Ortaköy → Kuruçeşme)
  TrackingSession _bosphorusJog() {
    final start = DateTime(2026, 3, 18, 7, 15);
    final waypoints = [
      (41.0480, 29.0270), // Ortaköy mosque area
      (41.0485, 29.0258),
      (41.0492, 29.0248),
      (41.0500, 29.0240),
      (41.0510, 29.0232),
  ...
  /// ~1.5 km walk around Sultanahmet historic area
  TrackingSession _sultanahmetWalk() {
    final start = DateTime(2026, 3, 16, 14, 30);
    final waypoints = [
      (41.0086, 28.9780), // Sultanahmet Square
      (41.0082, 28.9770),
      (41.0076, 28.9762),
      (41.0070, 28.9768),
      (41.0065, 28.9778),
      ...
      (41.0075, 28.9815),
      (41.0080, 28.9805),
      (41.0084, 28.9792), // Back near start
    ];
  ...
  /// ~3 km evening walk along Kadıköy waterfront
  TrackingSession _kadikoyWaterfront() {
    final start = DateTime(2026, 3, 15, 18, 0);
    final waypoints = [
      (40.9910, 29.0235), // Kadıköy pier
      (40.9905, 29.0248),
      (40.9898, 29.0260),
      (40.9890, 29.0272),
      (40.9882, 29.0285),
  ...
  /// ~2 km route from Bebek towards Rumeli Hisarı park
  TrackingSession _bebekToPark() {
    final start = DateTime(2026, 3, 14, 10, 0);
    final waypoints = [
      (41.0770, 29.0440), // Bebek
      (41.0778, 29.0432),
      (41.0785, 29.0420),
      (41.0793, 29.0408),
      (41.0800, 29.0395),
      ...
      (41.0835, 29.0335),
      (41.0842, 29.0325),
      (41.0850, 29.0315), // Near Rumeli Hisarı
    ];
  ...
  /// ~1 km short evening run near Maçka Park
  TrackingSession _shortEveningRun() {
    final start = DateTime(2026, 3, 17, 19, 45);
    final waypoints = [
      (41.0440, 28.9950), // Maçka Park entrance
      (41.0445, 28.9942),
      (41.0452, 28.9935),
      (41.0458, 28.9928),
      (41.0464, 28.9920),
  ...
  /// Interpolates extra points between waypoints and adds realistic noise.
  List<LocationPoint> _interpolateRoute(
    ...
    for (var i = 0; i < waypoints.length - 1; i++) {
      ...
      // Add 2 interpolated points between each waypoint pair
      for (var t = 0.0; t < 1.0; t += 0.33) {
        final lat = lat1 + (lat2 - lat1) * t + (rng.nextDouble() - 0.5) * 0.0001;
        final lon = lon1 + (lon2 - lon1) * t + (rng.nextDouble() - 0.5) * 0.0001;
        final speed =
